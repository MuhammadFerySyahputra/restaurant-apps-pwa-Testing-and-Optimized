...
<article tabindex="0" class="resto-item">
  <a href="/#/detail/${restaurant.id}">
    ...
    <p tabindex="0" class="resto-item_name" alt="nama restoran">${restaurant.name || '-'}</p>
      <p tabindex="0" class="resto-item_city" alt="kota restoran">${restaurant.city}
      </p>
      <span class="resto-item_rating" aria-label="rating resto ${restaurant.rating}">rating &star; ${restaurant.rating}</span>
  ...
  <div tabindex="0" class="container-info">
    ...
    <ul class="detail-info">
      ...
      <li class="resto-rating">
        <i title="ratings"></i>
        <p class="detail-name-address-rating">RATING &star; ${restaurant.rating}</p>
      </li>
      <li class="resto-category">${restaurant.categories.map((category) => `
        <span class="category">${category.name}</span>`).join('')}
      </li>
      <h4> Description: </h4>
      <li>
    ...
    .map(
      (food) => `
              <li><i class="bi bi-egg-fried"></i> ${food.name}</li>`,
    )
    .join('')}
            </ul>
          </div>
    ...
    .map(
      (drink) => `
              <li><i class="bi bi-cup-hot-fill"></i> ${drink.name}</li>`,
    )
    .join('')}
            </ul>
          </div>
